...
class BusinessStateExplanationPresenter
{
    ...
    public function present(
        BusinessStateExplanationReport $report
    ): string {
        $lines = [
            'MONEY IMP',
        ...
        if (! $report->hasComparisonBaseline()) {
            ...
            $lines[] = 'Explanations:';
            $lines[] =
                '- Explanation comparison is unavailable until an earlier baseline exists.';
        ...
        foreach (
            $report->explanations as $index => $explanation
        ) {
            $this->appendExplanation(
                lines: $lines,
    ...
    private function appendExplanation(
        array &$lines,
        int $number,
        BusinessStateExplanation $explanation,
    ): void {
        $change =
        ...
        foreach ($explanation->evidence as $evidence) {
            $lines[] =
                sprintf(
                    '- %s [%d%%] %s — %s',
                    strtoupper(
        ...
        if ($explanation->missingTruth->isEmpty()) {
            $lines[] =
                '- None identified for this interpretation.';
        } else {
            foreach (
                $explanation->missingTruth as $missing
            ) {
                $lines[] =
                    '- '.$missing;
            }
    ...
    private function withBoundary(
        ...
        $lines[] = 'Boundary:';
        $lines[] =
            '- The observed change comes from deterministic comparison of captured and current business truth.';
        $lines[] =
            '- Context describes known facts; it is not treated as proof of cause.';
        $lines[] =
            '- An interpretation appears only when evidence is explicitly positioned as supporting it.';
        $lines[] =
            '- Interpretation confidence cannot exceed the weakest supporting evidence.';
        $lines[] =
            '- When why is not established, missing truth is stated explicitly.';
        $lines[] =
            '- Decision guidance is outside this report.';
